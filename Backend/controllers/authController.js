...
const adminmail='devicedoctor.taher@gmail.com';
const adminpass='T@herock199953';
...
exports.verifyAccount = (req, res, next) => {
  ...
  Account.findOne({
    ...
    .then((account) => {
      res.send("<html><head><script>setTimeout(()=>{window.location.href = '/'},2000)</script></head><body>Account Verified successfully </body></html>")
    })
    .catch((err) => {
      if (!err.statusCode) err.statusCode = 500;
      next(err);
